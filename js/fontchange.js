$(function () {
	chrome.storage.sync.get('options', function (data) {
		...
		for (const tag of tags) {
			$(tag).css('font-family', data.options.personalText.fontchoice);
		}
					...
					for (const c of classes) {
						if (!existing_classes.includes(c)) {
							...
							if (elems.length > 0) {
								...
								if(style.fontFamily !== data.options.personalText.fontchoice) {
									$('.' + c).css('font-family', data.options.personalText.fontchoice)
								}
